...
@section('content')
    <div class="max-w-xl mx-auto mt-10" x-data="{
        countdown: 60,
        canResend: false,
        init() {
        ...
        <div class="bg-background p-8 rounded-lg shadow-md">
            ...
            <p class="text-center text-muted-foreground mb-6">We've sent a 6-digit OTP to your email address. Please enter
                the code below.</p>
            ...
            <form action="{{ route('otp.verify') }}" method="POST">
                ...
                <div class="mb-4">
                    <label for="otp_code" class="block text-sm font-medium text">OTP Code</label>
                    <input type="text" name="otp_code" id="otp_code"
                        class="w-full mt-1 px-4 py-2  required autofocus maxlength="6">
                    @error('otp_code')
                        <p class="text-destructive text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mt-6"><button type="submit" class="w-full bg-primary p-1 text-white">Verify Account</button>
                </div>
            </form>
            ...
            <div class="text-center mt-4 text-sm">
                ...
                <span x-show="!canResend" class=" text-secondary]">
